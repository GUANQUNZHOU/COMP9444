...
class LinearModel:
    ...
    def activation(self, x):
        ...
        try:
            input_x = float(x)
            sigmoid = 1/(1+np.exp(-input_x))
            return sigmoid
        except ValueError:
            print("The input accepted by activation function must be a float")
    ...
    def forward(self, inputs):
        ...
        bias = self.weights[-1]
        result = self.activation((np.dot(inputs,weight)+bias))
        return result
    @staticmethod
    def loss(prediction, label):
        ...
        """
        if label == 1:
            re = -np.log(prediction)
        else:
            re = -np.log((1-prediction))
        return re
    ...
    def backward(self, inputs, diff):
        ...
        """
        adjustment = self.lr*diff
        self.weights[:-1] = self.weights[:-1] + (adjustment * inputs)
        self.weights[-1] = self.weights[-1] + (adjustment * 1)
