...
class NNModel:
    ...
    def view_batch(self):
        ...
        image_1,label_1 = batch
        padding_value = 2
        dim = (image_1.shape[2]+padding_value)*8+padding_value
        image_t = torch.reshape(image_1,[8,8,image_1.shape[2],image_1.shape[2]]).numpy()
        image_batch = np.zeros([dim,dim])
        for i in range(dim):
            for j in range(dim):
                image_batch[i][j] = -1
        for out_x in range(image_t.shape[0]):
            for out_y in range(image_t.shape[1]):
                casual = image_t[out_x][out_y]
                base_x = out_x*(image_1.shape[2]+padding_value)+2
                base_y = out_y*(image_1.shape[2]+padding_value)+2
                for in_x in range(casual.shape[0]):
                    for in_y in range(casual.shape[1]):
                        image_batch[in_x+base_x][in_y+base_y] = casual[in_x][in_y]
        label_1 = torch.reshape(label_1,[8,8])
        return image_batch,label_1
